...
export default function ShowData() {
  ...
  const nextPage = () => {
    currentPage + 1 < numPages ? setCurrentPage(currentPage + 1) : console.log("already at the last page");
  }

  const prevPage = () => {
    currentPage > 0 ? setCurrentPage(currentPage - 1) : console.log("already at the first page");
  }

  const handleDelete = async (id, index) => {    
    if (!window.confirm("Wollen Sie den Eintrag wirklich entfernen?")) {
        return;
    }
    try {
              ...
              <tr>
                <td colSpan="5" className="text-center">
                  {kfzaData ? (
                    ...
                    <FontAwesomeIcon icon={faArrowRight} onClick={nextPage} className="spaceLeftSM" size="2x"/>
                    </>  
                  ) : ('')}
                </td>
              </tr>
