...
export default function NasaTable() {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "/api/fetchNASATLX?companyName=" + companyName
        );
        if (response.ok) {
  ...
  const columns = [
    ...
    { field: "mental", headerName: "Geistig", flex:1,  minWidth: 90, editable:true },
    { field: "physical", headerName: "Körperlich", flex:1,  minWidth: 90, editable:true },
    { field: "temporal", headerName: "Zeitlich", flex:1,  minWidth: 90, editable:true },
    { field: "performance", headerName: "Leistung", flex:1,  minWidth: 90, editable:true },
    { field: "effort", headerName: "Anstrengung", flex:1,  minWidth: 90, editable:true },
    { field: "frustration", headerName: "Frustration", flex:1,  minWidth: 90, editable:true },
    {
      field: "timestamp",
      headerName: "Datum",
      flex:1,
        ...
        rows.length !== -1 ? (
          <Box sx={{ height: 425, width: "auto" }}>
            <DataGrid
              ...
              slotProps={{toolbar:{csvOptions:{fileName: `NasaData${DateCamelCase()}`}}}}
              rows={rows}
              columns={columns}
              initialState={{
                pagination: {
